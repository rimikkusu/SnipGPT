...
It stays out of the way, listens for your hotkeys, lets you capture part of the screen, and sends the prompt to your selected AI provider. Version 2.2 improves the custom prompt workflow, long-answer handling, overlay usability, and overall quality-of-life.
...
- Scrollable answer box for long responses
...
Version 2.2 focuses on better long-answer support, cleaner overlays, and a smoother custom prompt experience.

### New in this version

- Added a **scrollable answer box**
  - long answers stay inside the overlay
  - you can scroll through them instead of having the overlay grow too large
- Added **text selection inside the answer box**
  - you can highlight and copy specific parts of an answer
- Improved the **Copy** button
  - it now copies the full returned answer
- Improved **long response handling**
  - better support for large code answers and longer explanations
- Improved the **custom prompt popup**
  - better support for pasting longer or multiline prompts
  - improved prompt box sizing and scrolling
- Improved **prompt popup controls**
  - `Enter` = prompt + screenshot
  - `Shift + Enter` = prompt only
  - `Esc` = cancel
- Added an **overlay hide combo**
  - `Esc + 1` hides the overlay without needing to click it first
- Improved overlay sizing
  - small status messages use a smaller overlay
  - long answers stay cleaner and easier to read
- Improved prompt fallback behavior
  - typed prompt first
  - saved custom prompt second
  - default built-in prompt last
...
5. Scroll, select text, or press **Copy** to copy the full result
...
- quick prompt-based explanations
- code help and longer written answers
...
- Long answers now stay inside a scrollable answer area instead of expanding across the screen
